...
def _pick_lang_value(payload: dict[str, Any], field: str, lang: str) -> Any:
    # payload is a dict; select <field>_<lang> with fallbacks
    ...
    candidates.extend(
        f'{field}_{code}'
        for code in settings.SUPPORTED_LANGS
        if code not in (lang, default_lang)
    )
...
def map_language_row(r: dict[str, Any], lang: str | None = None) -> LanguageBase:
    lang = lang or settings.DEFAULT_LANG
    return LanguageBase(
        id=str(r['id']),
        isocode=r['isocode'],
        name=_pick_lang_value(r, 'name', lang)
        or r.get('name_local')
        or r.get('name_en')
        or r.get('name_ru'),
        country=_pick_lang_value(r, 'country', lang)
        or r.get('country_en')
        or r.get('country_ru'),
...
def map_learning_language_row(
    r: dict[str, Any], lang: str | None = None
) -> LearningLanguageOut:
    lang = lang or settings.DEFAULT_LANG
    language_payload = {
