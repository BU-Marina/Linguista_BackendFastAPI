...
users_user_interests = Table(
    ...
    Column("interest_id", PG_UUID(as_uuid=True), ForeignKey("users_interest.id", ondelete="CASCADE"), primary_key=True),
)
...
users_user_cities = Table(
    ...
    Base.metadata,
    Column("user_id", PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), primary_key=True),
    Column("city_id", PG_UUID(as_uuid=True), ForeignKey("users_city.id", ondelete="CASCADE"), primary_key=True),
)
...
class User(SQLAlchemyBaseUserTable[uuid.UUID], Base, SlugMixin):
    ...
    profile_description = Column(String(UsersLengthLimits.PROFILE_DESCRIPTION_MAX_LENGTH), nullable=True)
    profile_header_image = Column(String(1024), nullable=True)

    # Teacher info
    is_teacher = Column(Boolean, nullable=False, default=False)
    teaching_goal = Column(String(UsersLengthLimits.TEACHING_GOAL_MAX_LENGTH), nullable=True)
    ...
    # FK
    study_plan_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_studyplan.id", ondelete="SET NULL"), nullable=True)

    # Relationships (FK)
    study_plan = relationship("StudyPlan", foreign_keys=[study_plan_id], back_populates="users", lazy="selectin")
...
class UserSettings(Base):
    ...
    allow_friends_requests_from = Column(String(FriendsRequestPermissionsEnum.max_length), nullable=False, default=FriendsRequestPermissionsEnum.ALL)
    allow_notifications_from = Column(String(FriendsNotificationsPermissionsEnum.max_length), nullable=False, default=FriendsNotificationsPermissionsEnum.ALL_FRIENDS)
    allow_send_notifications_to = Column(String(FriendsNotificationsPermissionsEnum.max_length), nullable=False, default=FriendsNotificationsPermissionsEnum.ALL_FRIENDS)
    allow_start_private_chat_for = Column(String(CommonPermissionsEnum.max_length), nullable=False, default=CommonPermissionsEnum.ALL)
    allow_invite_to_group_chat_for = Column(String(CommonPermissionsEnum.max_length), nullable=False, default=CommonPermissionsEnum.ALL)
    allow_collecting_stats_from = Column(String(ChatUsageAnalysPermissionEnum.max_length), nullable=False, default=ChatUsageAnalysPermissionEnum.ALL_CHATS)
    ...
    # Visibility
    show_achievements_for = Column(String(CommonPermissionsEnum.max_length), nullable=False, default=CommonPermissionsEnum.FRIENDS)
    show_certificates_for = Column(String(CommonPermissionsEnum.max_length), nullable=False, default=CommonPermissionsEnum.FRIENDS)
    show_online_status_for = Column(String(CommonPermissionsEnum.max_length), nullable=False, default=CommonPermissionsEnum.FRIENDS)
    show_strike_status_for = Column(String(CommonPermissionsEnum.max_length), nullable=False, default=CommonPermissionsEnum.FRIENDS)
    ...
    words_default_access_level = Column(String(8), nullable=True, default=AccessLevelsEnum.PUBLIC)
    collections_default_access_level = Column(String(8), nullable=True, default=AccessLevelsEnum.PUBLIC)

    # FK
    user_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False, unique=True)
...
class Subscription(Base):
    ...
    # Relationships (FK)
    subscriber = relationship("User", foreign_keys=[subscriber_id], backref="subscriptions_detail", lazy="selectin")
    user = relationship("User", foreign_keys=[user_id], backref="subscribers_detail", lazy="selectin")
...
class Interest(Base):
    ...
    name = Column(String(UsersLengthLimits.INTEREST_NAME_MAX_LENGTH), nullable=False, unique=True)
    cover = Column(String(1024), nullable=True)

    # FK
    author_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=True)
...
class Goal(Base):
    ...
    description = Column(String(UsersLengthLimits.GOAL_DESCRIPTION_MAX_LENGTH), nullable=True)
    ...
    # FK
    achievement_id = Column(PG_UUID(as_uuid=True), ForeignKey("achievements_achievement.id", ondelete="SET NULL"), nullable=True)
...
class Task(Base):
    ...
    # FK
    goal_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_goal.id", ondelete="CASCADE"), nullable=False)

    # Relationships (FK)
    goal = relationship("Goal", backref="tasks", lazy="selectin")

    __table_args__ = (
        Index("ix_users_task_created_modified", "created", "modified"),
    )
...
class TaskProgress(Base):
    ...
    task_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_task.id", ondelete="CASCADE"), nullable=False)
    user_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
...
class StudyPlan(Base):
    ...
    description = Column(String(UsersLengthLimits.STUDY_PLAN_DESCRIPTION_MAX_LENGTH), nullable=True)
    new_words_task = Column(SmallInteger, nullable=True)
    ...
    # Relationships (FK)
    author = relationship("User", foreign_keys=[author_id], backref="study_plans", lazy="selectin")
    users = relationship("User", foreign_keys=[User.study_plan_id], back_populates="study_plan", lazy="selectin")
...
class Certificate(Base):
    ...
    title = Column(String(UsersLengthLimits.CERTIFICATE_TITLE_MAX_LENGTH), nullable=False)
...
class ConfirmationRequest(Base):
    ...
    # FK
    certificate_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_certificate.id", ondelete="CASCADE"), nullable=False, unique=True)
    learning_language_id = Column(PG_UUID(as_uuid=True), ForeignKey("languages_userlearninglanguage.id", ondelete="CASCADE"), nullable=False)

    # Relationships (FK)
    certificate = relationship("Certificate", backref="level_confirm_requests", lazy="selectin")


class TeacherReview(Base):
    """..."""

    teacher_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
    text = Column(String(1024), nullable=False)
    stars = Column(SmallInteger, nullable=True)
    author_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)

    teacher = relationship("User", foreign_keys=[teacher_id], backref="reviews", lazy="selectin")
...
class MaterialGroup(Base):
    ...
    title = Column(String(UsersLengthLimits.MATERIAL_GROUP_TITLE_MAX_LENGTH), nullable=False)
...
class Material(Base):
    ...
    # FK
    material_group_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_materialgroup.id", ondelete="CASCADE"), nullable=False)
    author_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint("title", "author_id", name="unique_material"),
    )
...
class Grammar(Base, SlugMixin):
    ...
    construction_text = Column(String(UsersLengthLimits.GRAMMAR_TEXT_MAX_LENGTH), nullable=False)
...
class FavoriteGrammar(Base):
    ...
    # FK
    grammar_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_grammar.id", ondelete="CASCADE"), nullable=False)
    user_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
...
class Lesson(Base):
    ...
    description = Column(String(UsersLengthLimits.LESSON_DESCRIPTION_MAX_LENGTH), nullable=True)
    cover = Column(String(1024), nullable=True)
    read_access_level = Column(String(8), nullable=False, default="public")

    # FK
    exercises_set_id = Column(PG_UUID(as_uuid=True), ForeignKey("exercises_exercisesset.id", ondelete="SET NULL"), nullable=True)
    author_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
...
class LessonBlock(Base):
    ...
    title = Column(String(UsersLengthLimits.LESSON_BLOCK_TITLE_MAX_LENGTH), nullable=True)
    content = Column(String(UsersLengthLimits.LESSON_BLOCK_CONTENT_MAX_LENGTH), nullable=False)
    image = Column(String(1024), nullable=True)
    file = Column(String(1024), nullable=True)

    # FK
    lesson_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_lesson.id", ondelete="CASCADE"), nullable=False)
...
class StudyGroup(Base):
    ...
    # FK
    author_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
    chat_id = Column(PG_UUID(as_uuid=True), ForeignKey("chats_chat.id", ondelete="SET NULL"), nullable=True)
    vocabulary_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_collection.id", ondelete="SET NULL"), nullable=True)
...
class UserIssue(Base):
    ...
    # FK
    author_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
    word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="SET NULL"), nullable=True)
    goal_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_goal.id", ondelete="SET NULL"), nullable=True)
    collection_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_collection.id", ondelete="SET NULL"), nullable=True)
