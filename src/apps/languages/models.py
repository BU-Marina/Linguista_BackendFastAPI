...
class Language(Base):
    ...
    country = Column(String(256), nullable=True, server_default="")
    ...
    __table_args__ = (
        Index("ix_languages_language_sorting_name_isocode", "sorting", "name", "isocode"),
    )

    def __repr__(self):
        return f"<Language(name={self.name}, isocode={self.isocode})>"
...
class LanguageCoverImage(Base):
    ...
    # fields
    image = Column(String(1024), nullable=False)  # path to image
    default = Column(Boolean, nullable=False, server_default="false")

    # FK
    language_id = Column(PG_UUID(as_uuid=True), ForeignKey("languages_language.id", ondelete="CASCADE"), nullable=False)

    # Relationships (FK)
    language = relationship("Language", backref="images", lazy="selectin")

    __table_args__ = (
        Index("ix_languages_languagecoverimage_created", "created"),
    )
...
class UserLearningLanguage(Base, SlugMixin):
    ...
    is_taught = Column(Boolean, nullable=False, server_default="false", default=False)

    # FK
    language_id = Column(PG_UUID(as_uuid=True), ForeignKey("languages_language.id", ondelete="CASCADE"), nullable=False)
    user_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
    cover_id = Column(PG_UUID(as_uuid=True), ForeignKey("languages_languagecoverimage.id", ondelete="SET NULL"), nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint("language_id", "user_id", name="unique_user_learning_language"),
