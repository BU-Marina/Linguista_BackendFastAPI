...
achievements_users_access = Table(
    ...
    Column(
        ...
        ForeignKey("achievements_achievement.id", ondelete="CASCADE"),
        primary_key=True,
    ),
    Column(
        "user_id",
...
class Achievement(Base):
    ...
    # fields
    name = Column(
        String(AchievementsLengthLimits.ACHIEVEMENT_NAME_MAX_LENGTH), nullable=False
    )  # длину поставь из AchievementsLengthLimits.ACHIEVEMENT_NAME_MAX_LENGTH
    description = Column(
        String(AchievementsLengthLimits.ACHIEVEMENT_DESCRIPTION_MAX_LENGTH),
        nullable=True,
    )  # длину из AchievementsLengthLimits.ACHIEVEMENT_DESCRIPTION_MAX_LENGTH
    ...
    # FK
    achievement_group_id = Column(
        PG_UUID(as_uuid=True),
        ForeignKey("achievements_achievementgroup.id", ondelete="CASCADE"),
        nullable=True,
    )
    author_id = Column(
        PG_UUID(as_uuid=True),
    ...
    # Relationships (FK)
    achievement_group = relationship(
        "AchievementGroup", backref="achievements", lazy="selectin"
    )
    author = relationship("User", backref="achievements", lazy="selectin")
    users_access = relationship(
        "User",
        secondary=achievements_users_access,
        backref="achievements_available",
        lazy="selectin",
    )
    goals = relationship(
...
class AchievementGroup(Base):
    ...
    )  # длину из AchievementsLengthLimits.ACHIEVEMENT_GROUP_TITLE_MAX_LENGTH
    ...
    __table_args__ = (Index("ix_achievements_achievementgroup_created", "created"),)
...
class AchievementProgress(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "achievement_id", "user_id", name="unique_user_achievement_progress"
        ),
        Index(
            "ix_achievements_achievementprogress_created_modified",
            "created",
            "modified",
        ),
    )
