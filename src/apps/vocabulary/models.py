...
vocabulary_word_types = Table(
    ...
    Base.metadata,
    Column("word_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), primary_key=True),
    Column("wordtype_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_wordtype.id", ondelete="CASCADE"), primary_key=True),
)
...
vocabulary_collection_tags = Table(
    ...
    Base.metadata,
    Column("collection_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_collection.id", ondelete="CASCADE"), primary_key=True),
    Column("tag_id", PG_UUID(as_uuid=True), ForeignKey("core_tag.id", ondelete="CASCADE"), primary_key=True),
)
...
class Word(Base, SlugMixin):
    """Слово или фраза."""

    # fields
    text = Column(String(VocabularyLengthLimits.MAX_WORD_LENGTH), nullable=False)
    activity_status = Column(String(ActivityStatusEnum.max_length), nullable=False, server_default=ActivityStatusEnum.INACTIVE)
    ...
    # FK
    language_id = Column(PG_UUID(as_uuid=True), ForeignKey("languages_language.id", ondelete="SET NULL"), nullable=True)
    source_word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="SET NULL"), nullable=True)
    ...
    source_word = relationship("Word", remote_side="Word.id", backref="borrowings", lazy="selectin")
    author = relationship("User", backref="words", lazy="selectin")

    # Many-to-many (some via explicit through-models below)
    types = relationship("WordType", secondary=vocabulary_word_types, backref="words", lazy="selectin")
    tags = relationship("Tag", secondary=vocabulary_word_tags, backref="words", lazy="selectin")
...
class WordActivityHistory(Base):
    ...
    # FK
    session_id = Column(PG_UUID(as_uuid=True), ForeignKey("exercises_exercisesessionhistory.id", ondelete="CASCADE"), nullable=True)
    word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), nullable=False)

    # fields with choices stored as String
    previous_activity_status = Column(String(ActivityStatusEnum.max_length), nullable=False, server_default=ActivityStatusEnum.INACTIVE)
    new_activity_status = Column(String(ActivityStatusEnum.max_length), nullable=False, server_default=ActivityStatusEnum.ACTIVE)

    # Relationships (FK)
    session = relationship("ExerciseSessionHistory", backref="words_activity_changes", lazy="selectin")
    word = relationship("Word", backref="activity_history", lazy="selectin")

    __table_args__ = (
        Index("ix_vocabulary_wordactivityhistory_created", "created"),
    )
...
class WordType(Base, SlugMixin):
    ...
    __table_args__ = (
        Index("ix_vocabulary_wordtype_created", "created"),
    )
...
class FormGroup(Base, SlugMixin):
    ...
    # fields
    name = Column(String(VocabularyLengthLimits.MAX_FORMSGROUP_NAME_LENGTH), nullable=False)
    color = Column(String(7), nullable=True)
    translation = Column(String(VocabularyLengthLimits.MAX_FORMSGROUP_TRANSLATION_LENGTH), nullable=True)
...
class Definition(Base, SlugMixin):
    ...
    translation = Column(String(VocabularyLengthLimits.MAX_DEFINITION_LENGTH), nullable=True)
...
class UsageExample(Base, SlugMixin):
    ...
    translation = Column(String(VocabularyLengthLimits.MAX_EXAMPLE_LENGTH), nullable=True)
    source = Column(String(3), nullable=False, server_default="OTH")
    source_name = Column(String(VocabularyLengthLimits.MAX_EXAMPLE_SOURCE_LENGTH), nullable=True)
    source_url = Column(String(VocabularyLengthLimits.MAX_EXAMPLE_SOURCE_LINK_LENGTH), nullable=True)
...
class QuoteAssociation(Base):
    ...
    quote_author = Column(String(VocabularyLengthLimits.MAX_QUOTE_AUTHOR_LENGTH), nullable=True)
...
class Collection(Base, SlugMixin):
    """Коллекция."""

    # fields
    title = Column(String(VocabularyLengthLimits.MAX_COLLECTION_TITLE_LENGTH), nullable=False)
    description = Column(String(VocabularyLengthLimits.MAX_COLLECTION_DESCRIPTION_LENGTH), nullable=True)
    allow_comments = Column(Boolean, nullable=False, server_default="true")
    allow_suggestions = Column(Boolean, nullable=False, server_default="true")
    allow_suggestions_notifications = Column(Boolean, nullable=False, server_default="true")
    is_premium = Column(Boolean, nullable=False, server_default="false")

    # FK
    source_collection_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_collection.id", ondelete="SET NULL"), nullable=True)
    author_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)

    # Relationships (FK)
    source_collection = relationship("Collection", remote_side="Collection.id", backref="borrowings", lazy="selectin")
    ...
    tags = relationship("Tag", secondary=vocabulary_collection_tags, backref="collections", lazy="selectin")
    coauthors = relationship("User", secondary=vocabulary_collection_coauthors, backref="joint_collections", lazy="selectin")
...
class CollectionSubscription(Base):
    ...
    # FK
    subscriber_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
    collection_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_collection.id", ondelete="CASCADE"), nullable=False)

    # Relationships (FK)
    subscriber = relationship("User", backref="collections_subscriptions_detail", lazy="selectin")
    collection = relationship("Collection", backref="subscribers_detail", lazy="selectin")

    __table_args__ = (
        UniqueConstraint("subscriber_id", "collection_id", name="unique_collection_subscription"),
...
class WordsFormGroups(Base):
    ...
    # FK
    word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), nullable=False)
    forms_group_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_formgroup.id", ondelete="SET NULL"), nullable=True)

    # Relationships (FK)
    word = relationship("Word", backref="words_form_groups", lazy="selectin")
    forms_group = relationship("FormGroup", backref="words_form_groups", lazy="selectin")
...
class WordTranslations(Base):
    ...
    translation_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_wordtranslation.id", ondelete="CASCADE"), nullable=False)
    word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), nullable=False)

    translation = relationship("WordTranslation", backref="wordtranslations", lazy="selectin")
...
class WordDefinitions(Base):
    ...
    definition_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_definition.id", ondelete="CASCADE"), nullable=False)
    word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), nullable=False)
...
class WordUsageExamples(Base):
    ...
    example_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_usageexample.id", ondelete="CASCADE"), nullable=False)
    word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), nullable=False)
...
class WordImageAssociations(Base):
    ...
    image_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_imageassociation.id", ondelete="CASCADE"), nullable=False)
    word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), nullable=False)

    image = relationship("ImageAssociation", backref="wordimageassociations", lazy="selectin")
...
class WordQuoteAssociations(Base):
    quote_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_quoteassociation.id", ondelete="CASCADE"), nullable=False)
    word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), nullable=False)

    quote = relationship("QuoteAssociation", backref="wordquoteassociations", lazy="selectin")
...
class WordsInCollections(Base):
    ...
    collection = relationship("Collection", backref="words_in_collections", lazy="selectin")
...
class WordsSuggestedToCollections(Base):
    ...
    # fields
    status = Column(String(RequestStatusEnum.max_length), nullable=False, server_default=RequestStatusEnum.PENDING)
    ...
    __table_args__ = (
        UniqueConstraint("word_id", "collection_id", "user_id", name="unique_suggested_word"),
    )
...
class WordSelfRelationBase:
    ...
    def __table_args__(cls):
        ...
        return (
            UniqueConstraint("from_word_id", "to_word_id", name=f"unique_words_pair_{cls.__name__.lower()}"),
            CheckConstraint("from_word_id <> to_word_id", name=f"{cls.__name__.lower()}_not_same_word"),
        )
...
class FavoriteCollection(Base):
    ...
    __table_args__ = (
        UniqueConstraint("collection_id", "user_id", name="unique_user_favorite_collection"),
    )
...
class PremiumRequest(Base):
    ...
    request_status = Column(String(RequestStatusEnum.max_length), nullable=False, server_default=RequestStatusEnum.PENDING)

    # FK (one-to-one semantics in Django -> modeled as unique FK here)
    word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), nullable=True, unique=True)
    collection_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_collection.id", ondelete="CASCADE"), nullable=True, unique=True)
    author_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
...
vocabulary_collectioncomment_likes = Table(
    ...
    Base.metadata,
    Column("collectioncomment_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_collectioncomment.id", ondelete="CASCADE"), primary_key=True),
    Column("user_id", PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), primary_key=True),
)
...
vocabulary_wordcomment_likes = Table(
    ...
    Base.metadata,
    Column("wordcomment_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_wordcomment.id", ondelete="CASCADE"), primary_key=True),
    Column("user_id", PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), primary_key=True),
)
...
vocabulary_collectioncomment_answers = Table(
    ...
    Base.metadata,
    Column("collectioncomment_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_collectioncomment.id", ondelete="CASCADE"), primary_key=True),
    Column("answer_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_collectioncomment.id", ondelete="CASCADE"), primary_key=True),
)
...
vocabulary_wordcomment_answers = Table(
    ...
    Base.metadata,
    Column("wordcomment_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_wordcomment.id", ondelete="CASCADE"), primary_key=True),
    Column("answer_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_wordcomment.id", ondelete="CASCADE"), primary_key=True),
)
...
class CollectionComment(Base):
    ...
    likes = relationship("User", secondary=vocabulary_collectioncomment_likes, backref="collection_comments_liked", lazy="selectin")
    dislikes = relationship("User", secondary=vocabulary_collectioncomment_dislikes, backref="collection_comments_disliked", lazy="selectin")
    ...
    __table_args__ = (
        Index("ix_vocabulary_collectioncomment_created", "created"),
    )
...
class WordComment(Base):
    ...
    # M2M relationships (likes/dislikes/answers)
    likes = relationship("User", secondary=vocabulary_wordcomment_likes, backref="word_comments_liked", lazy="selectin")
    dislikes = relationship("User", secondary=vocabulary_wordcomment_dislikes, backref="word_comments_disliked", lazy="selectin")
    answers = relationship(
        "WordComment",
    ...
    __table_args__ = (
        Index("ix_vocabulary_wordcomment_created", "created"),
    )
