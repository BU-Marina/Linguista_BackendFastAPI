...
exercises_exercise_hints_available = Table(
    ...
    Base.metadata,
    Column("exercise_id", PG_UUID(as_uuid=True), ForeignKey("exercises_exercise.id", ondelete="CASCADE"), primary_key=True),
    Column("hint_id", PG_UUID(as_uuid=True), ForeignKey("exercises_hint.id", ondelete="CASCADE"), primary_key=True),
)
...
exercises_exerciseconfiguration_words_set = Table(
    ...
    Base.metadata,
    Column("exerciseconfiguration_id", PG_UUID(as_uuid=True), ForeignKey("exercises_exerciseconfiguration.id", ondelete="CASCADE"), primary_key=True),
    Column("wordsset_id", PG_UUID(as_uuid=True), ForeignKey("exercises_wordsset.id", ondelete="CASCADE"), primary_key=True),
)
...
exercises_exercisesessiontaskshistory_right_answers_words = Table(
    ...
    Base.metadata,
    Column("exercisesessiontaskshistory_id", PG_UUID(as_uuid=True), ForeignKey("exercises_exercisesessiontaskshistory.id", ondelete="CASCADE"), primary_key=True),
    Column("word_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), primary_key=True),
)
exercises_exercisesessiontaskshistory_right_answers_translations = Table(
    ...
    Base.metadata,
    Column("exercisesessiontaskshistory_id", PG_UUID(as_uuid=True), ForeignKey("exercises_exercisesessiontaskshistory.id", ondelete="CASCADE"), primary_key=True),
    Column("translation_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_wordtranslation.id", ondelete="CASCADE"), primary_key=True),
)
exercises_exercisesessiontaskshistory_right_answers_definitions = Table(
    ...
    Base.metadata,
    Column("exercisesessiontaskshistory_id", PG_UUID(as_uuid=True), ForeignKey("exercises_exercisesessiontaskshistory.id", ondelete="CASCADE"), primary_key=True),
    Column("definition_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_definition.id", ondelete="CASCADE"), primary_key=True),
)
...
exercises_customexerciseconfiguration_gaps = Table(
    ...
    Base.metadata,
    Column("customexerciseconfiguration_id", PG_UUID(as_uuid=True), ForeignKey("exercises_customexerciseconfiguration.id", ondelete="CASCADE"), primary_key=True),
    Column("customgap_id", PG_UUID(as_uuid=True), ForeignKey("exercises_customgap.id", ondelete="CASCADE"), primary_key=True),
)
...
exercises_exercisesset_words = Table(
    ...
    Base.metadata,
    Column("exerciseset_id", PG_UUID(as_uuid=True), ForeignKey("exercises_exercisesset.id", ondelete="CASCADE"), primary_key=True),
    Column("word_id", PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), primary_key=True),
)
...
class Exercise(Base, SlugMixin):
    ...
    constraint_description = Column(String(512), nullable=True)
    ...
    __table_args__ = (
        Index("ix_exercises_exercise_created", "created"),
    )
...
class Hint(Base):
    ...
    __table_args__ = (
        Index("ix_exercises_hint_created", "created"),
    )
...
class ExerciseSessionHistory(Base,):
    ...
    exercise_id = Column(PG_UUID(as_uuid=True), ForeignKey("exercises_exercise.id", ondelete="CASCADE"), nullable=False)
    ...
    __table_args__ = (
        Index("ix_exercises_exercisesessionhistory_created", "created"),
    )
...
class ExerciseSessionTasksHistory(Base):
    ...
    task_type = Column(String(TaskTypesEnum.max_length), nullable=False, server_default=TaskTypesEnum.TEXT)
    task_language = Column(String(8), nullable=True)
    task_index = Column(SmallInteger, nullable=False)
    answer = Column(String(ExercisesLengthLimits.ANSWER_TEXT_MAX_LENGTH), nullable=False)
    ...
    # FK
    session_id = Column(PG_UUID(as_uuid=True), ForeignKey("exercises_exercisesessionhistory.id", ondelete="CASCADE"), nullable=False)
    task_word_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_word.id", ondelete="CASCADE"), nullable=False)
    task_translation_id = Column(PG_UUID(as_uuid=True), ForeignKey("vocabulary_wordtranslation.id", ondelete="CASCADE"), nullable=True)
    ...
    task_translation = relationship("WordTranslation", backref="exercises_history", lazy="selectin")

    # Many-to-many relationships
    right_answers_words = relationship("Word", secondary=exercises_exercisesessiontaskshistory_right_answers_words, backref="tasks_history", lazy="selectin")
    right_answers_translations = relationship("WordTranslation", secondary=exercises_exercisesessiontaskshistory_right_answers_translations, backref="tasks_history", lazy="selectin")
    right_answers_definitions = relationship("Definition", secondary=exercises_exercisesessiontaskshistory_right_answers_definitions, backref="tasks_history", lazy="selectin")

    hints_available = relationship("Hint", secondary=exercises_exercisesessiontaskshistory_hints_available, backref="sessions", lazy="selectin")
    hints_used = relationship("Hint", secondary=exercises_exercisesessiontaskshistory_hints_used, backref="history", lazy="selectin")

    __table_args__ = (
        Index("ix_exercises_exercisesessiontaskshistory_created_taskindex", "created", "task_index"),
    )
...
class ExerciseConfiguration(Base):
    ...
    # fields
    input_mode = Column(String(ExercisesInputModeEnum.max_length), nullable=False, server_default=ExercisesInputModeEnum.FREE_INPUT)
    answer_time_limit = Column(SmallInteger, nullable=True)
    time_limit_mode = Column(String(TimeLimitModeEnum.max_length), nullable=False, server_default=TimeLimitModeEnum.ALL)
    repetitions_amount = Column(SmallInteger, nullable=False, server_default="1")
    translations_mode = Column(String(TranslationsModeEnum.max_length), nullable=True, server_default=TranslationsModeEnum.FROM_LEARNING)
    definitions_mode = Column(String(DefinitionsModeEnum.max_length), nullable=True, server_default=DefinitionsModeEnum.DEFINITION_BY_WORD)
    ...
    # Many-to-many relationships
    words_set = relationship("WordsSet", secondary=exercises_exerciseconfiguration_words_set, backref="exercise_configurations", lazy="selectin")
    words = relationship("Word", secondary=exercises_exerciseconfiguration_words, backref="exercise_configurations", lazy="selectin")
    hints_available = relationship("Hint", secondary=exercises_exerciseconfiguration_hints_available, backref="exercise_configurations", lazy="selectin")

    __table_args__ = (
        Index("ix_exercises_exerciseconfiguration_created", "created"),
    )
...
class WordsSet(Base, SlugMixin):
    ...
    # fields
    name = Column(String(ExercisesLengthLimits.WORDS_SET_NAME_MAX_LENGTH), nullable=False)
    ...
    # Many-to-many
    words = relationship("Word", secondary=exercises_wordsset_words, backref="sets", lazy="selectin")

    __slug_source__ = ["name", "exercise__name", "author__username"]

    __table_args__ = (
        Index("ix_exercises_wordsset_lastexercise_created", "last_exercise_date", "created"),
...
class CustomExerciseConfiguration(Base):
    ...
    # fields
    task = Column(String(ExercisesLengthLimits.EXERCISE_TASK_MAX_LENGTH), nullable=False)
    content_text = Column(String(ExercisesLengthLimits.EXERCISE_TASK_CONTENT_MAX_LENGTH), nullable=True)
    ...
    # Many-to-many: gaps
    gaps = relationship("CustomGap", secondary=exercises_customexerciseconfiguration_gaps, backref="custom_exercises", lazy="selectin")
...
class CustomGap(Base):
    ...
    # Many-to-many
    allowed_words = relationship("Word", secondary=exercises_customgap_allowed_words, backref="custom_exercises_gaps", lazy="selectin")

    __table_args__ = (
        Index("ix_exercises_customgap_created", "created"),
    )
...
class ExercisesSet(Base, SlugMixin):
    ...
    # fields
    title = Column(String(ExercisesLengthLimits.EXERCISES_SET_TITLE_MAX_LENGTH), nullable=False)
    run_access_level = Column(String(AccessLevelsEnum.max_length), nullable=False, server_default=AccessLevelsEnum.PUBLIC)
    add_access_level = Column(String(AccessLevelsEnum.max_length), nullable=False, server_default=AccessLevelsEnum.PUBLIC)
    ...
    # Many-to-many
    words = relationship("Word", secondary=exercises_exercisesset_words, backref="exercises_sets", lazy="selectin")
    exercises = relationship("ExerciseConfiguration", secondary=exercises_exercisesset_exercises, backref="exercises_sets", lazy="selectin")
    custom_exercises = relationship("CustomExerciseConfiguration", secondary=exercises_exercisesset_custom_exercises, backref="exercises_sets", lazy="selectin")
...
class FavoriteExerciseSet(Base):
    ...
    # FK
    exercises_set_id = Column(PG_UUID(as_uuid=True), ForeignKey("exercises_exercisesset.id", ondelete="CASCADE"), nullable=False)
    user_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)

    # Relationships (FK)
    exercises_set = relationship("ExercisesSet", backref="favorite_for", lazy="selectin")
    user = relationship("User", backref="favorite_exercises_sets", lazy="selectin")

    __table_args__ = (
        UniqueConstraint("exercises_set_id", "user_id", name="unique_favorite_exercises_set"),
...
class ExerciseSchedule(Base):
    ...
    # FK
    user_id = Column(PG_UUID(as_uuid=True), ForeignKey("users_user.id", ondelete="CASCADE"), nullable=False)
    exercise_id = Column(PG_UUID(as_uuid=True), ForeignKey("exercises_exercise.id", ondelete="CASCADE"), nullable=True)
    exercise_configuration_id = Column(PG_UUID(as_uuid=True), ForeignKey("exercises_exerciseconfiguration.id", ondelete="CASCADE"), nullable=True)
    exercises_set_id = Column(PG_UUID(as_uuid=True), ForeignKey("exercises_exercisesset.id", ondelete="CASCADE"), nullable=True)

    # Relationships (FK)
    user = relationship("User", backref="schedule", lazy="selectin")
    exercise = relationship("Exercise", backref="schedule", lazy="selectin")
    exercise_configuration = relationship("ExerciseConfiguration", backref="schedule", lazy="selectin")
    exercises_set = relationship("ExercisesSet", backref="schedule", lazy="selectin")

    __table_args__ = (
        Index("ix_exercises_schedule_scheduled_created_modified", "scheduled_datetime", "created", "modified"),
    )
